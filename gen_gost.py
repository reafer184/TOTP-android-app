...
from gostcrypto.gosthash.gost_34_11_2012 import _T, GOST34112012

flat = bytearray()
for k in range(8):
    for x in range(256):
        flat += _T[k][x].to_bytes(8, 'little')
B64 = base64.b64encode(flat).decode()
...
def lps(d):
    ...
    for col in range(8):
        ...
        for k in range(8):
            t = (k*256+d[col+k*8])*8
            v = int.from_bytes(flat[t:t+8],'little')
            cur = int.from_bytes(out[base:base+8],'little')
            out[base:base+8] = (cur^v).to_bytes(8,'little')
    return bytes(out)
assert lps(bytes(range(64))) == bytes(GOST34112012._hash_lps(bytearray(range(64)))), "T-tables mismatch!"
print("[OK] T-tables verified")

with urllib.request.urlopen(
    "https://raw.githubusercontent.com/reafer184/TOTP-android-app"
    "/27a25f8b6abbe5016fb9a80c2ada8373c25b233c/pwa/gost.js"
) as r:
    orig = r.read().decode()
lines = orig.split("\n")
gh_start = next(i for i,l in enumerate(lines) if "GOST R 34.12" in l and "\u2500" in l)
gh_section = "\n".join(lines[gh_start:])

C_VALS = [
  "[0x07,0x45,0xa6,0xf2,0x59,0x65,0x80,0xdd,0x23,0x4d,0x74,0xcc,0x36,0x74,0x76,0x05,0x15,0xd3,0x60,0xa4,0x08,0x2a,0x42,0xa2,0x01,0x69,0x67,0x92,0x91,0xe0,0x7c,0x4b,0xfc,0xc4,0x85,0x75,0x8d,0xb8,0x4e,0x71,0x16,0xd0,0x45,0x2e,0x43,0x76,0x6a,0x2f,0x1f,0x7c,0x65,0xc0,0x81,0x2f,0xcb,0xeb,0xe9,0xda,0xca,0x1e,0xda,0x5b,0x08,0xb1]",
  ...
  "[0xb2,0x0a,0xba,0x0a,0xf5,0x96,0x1e,0x99,0x31,0xdb,0x7a,0x86,0x43,0xf4,0xb6,0xc2,0x09,0xdb,0x62,0x60,0x37,0x3a,0xc9,0xc1,0xb1,0x9e,0x35,0x90,0xe4,0x0f,0xe2,0xd3,0x7b,0x7b,0x29,0xb1,0x14,0x75,0xea,0xf2,0x8b,0x1f,0x9c,0x52,0x5f,0x5e,0xf1,0x06,0x35,0x84,0x3d,0x6a,0x28,0xfc,0x39,0x0a,0xc7,0x2f,0xce,0x2b,0xac,0xdc,0x74,0xf5]",
  ...
  "[0xed,0x9c,0x45,0x98,0xfb,0xc7,0xb4,0x74,0xc3,0xb6,0x3b,0x15,0xd1,0xfa,0x98,0x36,0xf4,0x52,0x76,0x3b,0x30,0x6c,0x1e,0x7a,0x4b,0x33,0x69,0xaf,0x02,0x67,0xe7,0x9f,0x03,0x61,0x33,0x1b,0x8a,0xe1,0xff,0x1f,0xdb,0x78,0x8a,0xff,0x1c,0xe7,0x41,0x89,0xf3,0xf3,0xe4,0xb2,0x48,0xe5,0x2a,0x38,0x52,0x6f,0x05,0x80,0xa6,0xde,0xbe,0xab]",
  "[0x1b,0x2d,0xf3,0x81,0xcd,0xa4,0xca,0x6b,0x5d,0xd8,0x6f,0xc0,0x4a,0x59,0xa2,0xde,0x98,0x6e,0x47,0x7d,0x1d,0xcd,0xba,0xef,0xca,0xb9,0x48,0xea,0xef,0x71,0x1d,0x8a,0x79,0x66,0x84,0x14,0x21,0x80,0x01,0x20,0x61,0x07,0xab,0xeb,0xbb,0x6b,0xfa,0xd8,0x94,0xfe,0x5a,0x63,0xcd,0xc6,0x02,0x30,0xfb,0x89,0xc8,0xef,0xd0,0x9e,0xcd,0x7b]",
  "[0x20,0xd7,0x1b,0xf1,0x4a,0x92,0xbc,0x48,0x99,0x1b,0xb2,0xd9,0xd5,0x17,0xf4,0xfa,0x52,0x28,0xe1,0x88,0xaa,0xa4,0x1d,0xe7,0x86,0xcc,0x91,0x18,0x9d,0xef,0x80,0x5d,0x9b,0x9f,0x21,0x30,0xd4,0x12,0x20,0xf8,0x77,0x1d,0xdf,0xbc,0x32,0x3c,0xa4,0xcd,0x7a,0xb1,0x49,0x04,0xb0,0x80,0x13,0xd2,0xba,0x31,0x16,0xf1,0x67,0xe7,0x8e,0x37]",
]
C_JS = ",\n".join("  new Uint8Array(" + v + ")" for v in C_VALS)
...
content = TMPL.replace("__B64__", B64).replace("__C_JS__", C_JS).replace("__GH__", gh_section)
with open("pwa/gost.js", "w") as f:
    f.write(content)
print(f"[OK] pwa/gost.js: {len(content)} chars")
